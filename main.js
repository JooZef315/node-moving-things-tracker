...
arrayTemp.pop();
var pathToTrackerOutput =
  arrayTemp.length > 0 ? `${arrayTemp.join("/")}/tracker.json` : `tracker.json`;
...
fs.readFile(`${pathRawDetectionsInput}`, function (err, f) {
  ...
  if (MODE_DARKNET) {
    ...
    try {
      ...
      detectionsFromDarknet.map((detectionFromDarknet) => {
        detections[detectionFromDarknet.frame_id] =
          detectionFromDarknet.objects.map((objectFromDarknet) => {
            return {
      ...
      console.log(
        "Error while parsing JSON input file, make sure it is valid json"
      );
      return;
    }
  } else {
  ...
  Object.keys(detections).forEach(function (frameNb) {
    ...
    if (MODE_BEATTHETRAFFIC) {
      ...
      detectionsForThisFrame = detectionsForThisFrame.filter(
        (detection) => !isInsideSomeAreas(IGNORED_AREAS, detection)
      );
      // Remove unwanted items
      detectionsForThisFrame = ignoreObjectsNotToDetect(
      ...
      detectionsForThisFrame = detectionsForThisFrame.filter(
        (detection) =>
          !isDetectionTooLarge(detection, LARGEST_DETECTION_ALLOWED)
      );
    }
